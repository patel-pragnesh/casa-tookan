module.exports = function () {
	...
	self.checkPasscode = function (params) {
			...
			if (info.easypass != null) {
				...
				function input() {
					...
					dialog.origin.addEventListener("click", function (e) {
						var inputText = false ? dialog.androidField.value : e.text;
		...
		if (false) input_password();else {
			if (cache.data.isTouchId != null) {
				TiTouchId.authenticate({
					"reason": L("label_fingerprint"),
					"callback": function (e) {
						if (e.success) params.callback({ success: true, reason: self.REASON_TOUCHID });else {
							if (e.code == TiTouchId.ERROR_USER_CANCEL) params.callback({ success: false, reason: self.REASON_CANCEL });else input_password();
						}
					}
				});
			} else input_password();
	...
	self.useTouchID = function (params) {
		TiTouchId.authenticate({
			...
			callback: function (e) {
				...
				if (e.success) params.callback({ "success": true });else params.callback({ "success": false });
			}
		});
	};
