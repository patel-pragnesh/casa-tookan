	...
	self.check = function(){
		for(var i = 0; i < self.verifies.length; i++ ){
			...
			if( verify.type === "number" ){
				if( isNaN(checkValue) ){
					return {target: verify.target, message: L("label_inputverify_number").format({ "name": verify.name })};
				}
			}
			if( "equal" in verify ){
				if( checkValue != verify.equal.value ) return {target: verify.target, message: L("label_inputverify_equal").format({ "name": verify.name })};
			}
			if( "over" in verify ){
				if( verify.over == 0 && checkValue.length <= 0 ){
					return {target: verify.target, message: L("label_inputverify_empty").format({ "name": verify.name })};
				}
				if( checkValue.length < verify.over ){
					return {target: verify.target, message: L("label_inputverify_more").format({ "name": verify.name, "over": verify.over })};
				}
			}
			if( "shouldvalue" in verify ){
				if( checkValue <= 0 ){
					return {target: verify.target, message: L("label_inputverify_shouldvalue").format({ "name": verify.name })};
				}
			}
		}
		return true;
