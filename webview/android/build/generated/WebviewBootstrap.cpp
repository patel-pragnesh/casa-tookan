...
#define TAG "com.indiesquare.webview"

using namespace v8;

static Persistent<Object> bindingCache;

static void Webview_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "Webview.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = ::WebviewBindings::lookupGeneratedInit(
...
static void Webview_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, Webview_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void Webview_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::WebviewBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry WebviewBinding = {
	"com.indiesquare.webview",
	Webview_init,
	Webview_dispose
};
...
Java_com_indiesquare_webview_WebviewBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.indiesquare.webview", &WebviewBinding);
	titanium::KrollBindings::addExternalLookup(&(::WebviewBindings::lookupGeneratedInit));
}
