 ...
 * @class
 */
function CAF_SMPTE_Time (pointer) {
	if (pointer) {
		var oldWrapper = Hyperloop.getWrapper(pointer);
		if (oldWrapper) return oldWrapper;
	}
	if (!(this instanceof CAF_SMPTE_Time)) { throw new TypeError('Cannot instantiate a class by calling it as a function'); }
	if (!$init) {
		$initialize();
	}
	
	if (!pointer) {
		pointer = Hyperloop.createPointer('{CAF_SMPTE_Time=ccccI}', 'AudioToolbox', '/Applications/Xcode.app/Contents/Developer/Platforms/iPhoneSimulator.platform/Developer/SDKs/iPhoneSimulator11.3.sdk/System/Library/Frameworks/AudioToolbox.framework/Headers/CAFFile');
	}
...
function $initialize () {
	...
	CAF_SMPTE_Time._imports = $imports;

	// properties
	Object.defineProperties(CAF_SMPTE_Time.prototype, {
