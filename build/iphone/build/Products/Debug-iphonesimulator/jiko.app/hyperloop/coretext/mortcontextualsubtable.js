 ...
 * @class
 */
function MortContextualSubtable (pointer) {
	if (pointer) {
		var oldWrapper = Hyperloop.getWrapper(pointer);
		if (oldWrapper) return oldWrapper;
	}
	if (!(this instanceof MortContextualSubtable)) { throw new TypeError('Cannot instantiate a class by calling it as a function'); }
	if (!$init) {
		$initialize();
	}
	this.$header = {};
	...
	if (!pointer) {
		pointer = Hyperloop.createPointer('{MortContextualSubtable={STHeader=CCSSS}S}', 'CoreText', '/Applications/Xcode.app/Contents/Developer/Platforms/iPhoneSimulator.platform/Developer/SDKs/iPhoneSimulator11.3.sdk/System/Library/Frameworks/CoreText.framework/Headers/SFNTLayoutTypes');
	}
...
function $initialize () {
	...
	MortContextualSubtable._imports = $imports;

	// properties
	Object.defineProperties(MortContextualSubtable.prototype, {
...
module.exports = MortContextualSubtable;
