 ...
 * @class
 */
function DenseMatrix_Double (pointer) {
	if (pointer) {
		var oldWrapper = Hyperloop.getWrapper(pointer);
		if (oldWrapper) return oldWrapper;
	}
	if (!(this instanceof DenseMatrix_Double)) { throw new TypeError('Cannot instantiate a class by calling it as a function'); }
	if (!$init) {
		$initialize();
	}
	this.$attributes = {};
	...
	if (!pointer) {
		pointer = Hyperloop.createPointer('{DenseMatrix_Double=iii{?=b1b1b2b11b1}^d}', 'Accelerate', '/Applications/Xcode.app/Contents/Developer/Platforms/iPhoneSimulator.platform/Developer/SDKs/iPhoneSimulator11.3.sdk/System/Library/Frameworks/Accelerate.framework/Frameworks/vecLib.framework/Headers/Sparse/Solve');
	}
...
function $initialize () {
	...
	DenseMatrix_Double._imports = $imports;

	// properties
	Object.defineProperties(DenseMatrix_Double.prototype, {
...
module.exports = DenseMatrix_Double;
