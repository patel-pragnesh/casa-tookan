...
@interface HyperloopCoreMedia : NSObject
@end

@implementation HyperloopCoreMedia




+ (id) Block_OSStatus_____CMSampleBufferRef__CMItemCount_:(KrollCallback *) callback {
	return [^(CMSampleBufferRef arg0, CMItemCount arg1) {
		void(^Callback)(void) = ^{
			NSArray *args = nil;
...
+ (id) Block_void_____struct_opaqueCMSampleBuffer___:(KrollCallback *) callback {
	return [^(struct opaqueCMSampleBuffer * arg0) {
		void(^Callback)(void) = ^{
			NSArray *args = nil;
			id _arg0 = (arg0 == nil) ? (id)[NSNull null] : (id)[HyperloopPointer pointer:(const void *)&arg0 encoding:@encode(struct opaqueCMSampleBuffer *)];
			args = @[_arg0];
			[HyperloopUtils invokeCallback:callback args:args thisObject:callback];
		};
		if ([NSThread isMainThread]) {
