 ...
 * @class
 */
function ExtendedAudioFormatInfo (pointer) {
	if (pointer) {
		var oldWrapper = Hyperloop.getWrapper(pointer);
		if (oldWrapper) return oldWrapper;
	}
	if (!(this instanceof ExtendedAudioFormatInfo)) { throw new TypeError('Cannot instantiate a class by calling it as a function'); }
	if (!$init) {
		$initialize();
	}
	this.$mASBD = {};
	...
	this.$mClassDescription = {};
	if (pointer) {
		this.$mClassDescription.mType = $dispatch(pointer, 'valueAtIndex:', 9);
	}
	if (pointer) {
		this.$mClassDescription.mSubType = $dispatch(pointer, 'valueAtIndex:', 10);
	}
	if (pointer) {
		this.$mClassDescription.mManufacturer = $dispatch(pointer, 'valueAtIndex:', 11);
	}
	if (!pointer) {
		pointer = Hyperloop.createPointer('{ExtendedAudioFormatInfo={AudioStreamBasicDescription=dIIIIIIII}r^vI{AudioClassDescription=III}}', 'AudioToolbox', '/Applications/Xcode.app/Contents/Developer/Platforms/iPhoneSimulator.platform/Developer/SDKs/iPhoneSimulator11.3.sdk/System/Library/Frameworks/AudioToolbox.framework/Headers/AudioFormat');
	}
	...
	Object.defineProperty(this.$mClassDescription, 'mType', {
		set: function (_value) {
	...
	Object.defineProperty(this.$mClassDescription, 'mSubType', {
		set: function (_value) {
	...
	Object.defineProperty(this.$mClassDescription, 'mManufacturer', {
		set: function (_value) {
...
function $initialize () {
	...
	$imports.AudioClassDescription = require('/hyperloop/coreaudio/audioclassdescription');

	ExtendedAudioFormatInfo._imports = $imports;

	// properties
	Object.defineProperties(ExtendedAudioFormatInfo.prototype, {
		...
		mClassDescription: {
			...
			set: function (_mClassDescription) {
				this.$mClassDescription.mType = _mClassDescription.mType;
				this.$mClassDescription.mSubType = _mClassDescription.mSubType;
				this.$mClassDescription.mManufacturer = _mClassDescription.mManufacturer;
			},
			enumerable: false
		}
...
module.exports = ExtendedAudioFormatInfo;
