 ...
 * @class
 */
function CFRunLoopSourceContext (pointer) {
	if (pointer) {
		var oldWrapper = Hyperloop.getWrapper(pointer);
		if (oldWrapper) return oldWrapper;
	}
	if (!(this instanceof CFRunLoopSourceContext)) { throw new TypeError('Cannot instantiate a class by calling it as a function'); }
	if (!$init) {
		$initialize();
	}
	
	if (!pointer) {
		pointer = Hyperloop.createPointer('{CFRunLoopSourceContext=q^v^?^?^?^?^?^?^?^?}', 'CoreFoundation', '/Applications/Xcode.app/Contents/Developer/Platforms/iPhoneSimulator.platform/Developer/SDKs/iPhoneSimulator11.3.sdk/System/Library/Frameworks/CoreFoundation.framework/Headers/CFRunLoop');
	}
...
function $initialize () {
	...
	CFRunLoopSourceContext._imports = $imports;

	// properties
	Object.defineProperties(CFRunLoopSourceContext.prototype, {
		...
		schedule: {
			...
			set: function (_schedule) {
				$dispatch(this.$native, 'setValue:atIndex:', [_schedule, 7]);
			},
			enumerable: false
		},
		...
		cancel: {
			...
			set: function (_cancel) {
				$dispatch(this.$native, 'setValue:atIndex:', [_cancel, 8]);
			},
			enumerable: false
		},
		...
		perform: {
			...
			set: function (_perform) {
				$dispatch(this.$native, 'setValue:atIndex:', [_perform, 9]);
			},
			enumerable: false
		}
...
module.exports = CFRunLoopSourceContext;
