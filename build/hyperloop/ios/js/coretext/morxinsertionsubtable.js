 ...
 * @class
 */
function MorxInsertionSubtable (pointer) {
	if (pointer) {
		var oldWrapper = Hyperloop.getWrapper(pointer);
		if (oldWrapper) return oldWrapper;
	}
	if (!(this instanceof MorxInsertionSubtable)) { throw new TypeError('Cannot instantiate a class by calling it as a function'); }
	if (!$init) {
		$initialize();
	}
	this.$header = {};
	...
	if (!pointer) {
		pointer = Hyperloop.createPointer('{MorxInsertionSubtable={STXHeader=IIII}I}', 'CoreText', '/Applications/Xcode.app/Contents/Developer/Platforms/iPhoneSimulator.platform/Developer/SDKs/iPhoneSimulator11.3.sdk/System/Library/Frameworks/CoreText.framework/Headers/SFNTLayoutTypes');
	}
...
function $initialize () {
	...
	MorxInsertionSubtable._imports = $imports;

	// properties
	Object.defineProperties(MorxInsertionSubtable.prototype, {
		...
		insertionGlyphTableOffset: {
			...
			set: function (_insertionGlyphTableOffset) {
				$dispatch(this.$native, 'setValue:atIndex:', [_insertionGlyphTableOffset, 1]);
			},
			enumerable: false
		}
...
module.exports = MorxInsertionSubtable;
