 ...
 * @class
 */
function MorxRearrangementSubtable (pointer) {
	if (pointer) {
		var oldWrapper = Hyperloop.getWrapper(pointer);
		if (oldWrapper) return oldWrapper;
	}
	if (!(this instanceof MorxRearrangementSubtable)) { throw new TypeError('Cannot instantiate a class by calling it as a function'); }
	if (!$init) {
		$initialize();
	}
	this.$header = {};
	...
	if (!pointer) {
		pointer = Hyperloop.createPointer('{MorxRearrangementSubtable={STXHeader=IIII}}', 'CoreText', '/Applications/Xcode.app/Contents/Developer/Platforms/iPhoneSimulator.platform/Developer/SDKs/iPhoneSimulator11.3.sdk/System/Library/Frameworks/CoreText.framework/Headers/SFNTLayoutTypes');
	}
...
function $initialize () {
	...
	MorxRearrangementSubtable._imports = $imports;

	// properties
	Object.defineProperties(MorxRearrangementSubtable.prototype, {
...
module.exports = MorxRearrangementSubtable;
