 ...
 * @class
 */
function SparseMatrix_Double (pointer) {
	if (pointer) {
		var oldWrapper = Hyperloop.getWrapper(pointer);
		if (oldWrapper) return oldWrapper;
	}
	if (!(this instanceof SparseMatrix_Double)) { throw new TypeError('Cannot instantiate a class by calling it as a function'); }
	if (!$init) {
		$initialize();
	}
	this.$structure = {};
	...
	if (!pointer) {
		pointer = Hyperloop.createPointer('{SparseMatrix_Double={?=ii^q^i{?=b1b1b2b11b1}C}^d}', 'Accelerate', '/Applications/Xcode.app/Contents/Developer/Platforms/iPhoneSimulator.platform/Developer/SDKs/iPhoneSimulator11.3.sdk/System/Library/Frameworks/Accelerate.framework/Frameworks/vecLib.framework/Headers/Sparse/Solve');
	}
...
function $initialize () {
	...
	SparseMatrix_Double._imports = $imports;

	// properties
	Object.defineProperties(SparseMatrix_Double.prototype, {
...
module.exports = SparseMatrix_Double;
